...
#### Recent function changes (2026-08-01)
- **`deconvolve`**: gained `output_tag=''`, an optional string inserted into the output filename (before the `psf_model`/`algorithm` suffixes) to distinguish runs on a different **input variant** of the same series/channel (e.g. a bit-depth-quantized copy) from the standard run — without it, running identical `psf_model`/`algorithm`/`num_iter` settings on different input would silently overwrite the standard output, since the filename doesn't otherwise encode what was fed in. Empty by default; no effect on any existing call.
- **`deconvolve`**: `psf_model='estimate'`'s blind PSF pre-pass (representative middle Z-frame) can degenerate to an all-zero recovered kernel if that frame has no usable signal (observed with aggressively bit-depth-quantized input — real background values collapse to exactly 0 under `stack >> 8`, essentially never happens at full precision). Previously this silently produced `NaN` and crashed several calls later with an opaque `"cannot convert float NaN to integer"`. Now raises a specific, catchable `RuntimeError` at the point of failure instead. Deliberately **no fallback to the theoretical sigma** and **no change to which frame is picked** — either would make runs on different input variants algorithmically different from each other, not just different in input, which would confound any comparison between them (e.g. the bit-depth check above). Callers doing batch/parallel runs across many scene/channels should catch this per-job and skip, not let it abort the whole batch — see the pattern in `26_06_15_all_b.ipynb`'s/`26_06_25_all_b.ipynb`'s 8-bit comparison cells.

#### Recent function changes (2026-09-03)
- **Bug fix in `fast_plotter` (`src/plotter.py`)**: the significance-bracket geometry (y-offset above the tallest bar, spacing between stacked brackets, bracket tick height, and the auto y-limit headroom) was hardcoded in absolute data units tuned for ~0–1-scale metrics (`+0.04`, step `0.04`, height `0.015`, `+0.1` headroom). On any larger y-scale — raw intensities, counts — the brackets landed on top of the tallest bar with the stars hidden behind the title. All four are now fractions of the plotted span (`0.08 / 0.09 / 0.025 / 0.14 × span`), so brackets render with headroom at any scale. Explicit `ylim=` still overrides. First hit in `analysis_notebooks/26_07_31_Lisa_MB_pooled_intensity.ipynb`.
...
- **Bit-depth quantization** — confirmed systematically (2026-08-01), not just on the single sample originally checked. Quantizing every raw `2026_06_15`/`2026_06_25` stack to 8-bit-equivalent (`stack >> 8`, simulating a coarser ADC at the same gain) and re-running the full pipeline (`blind_richardson_lucy`, `psf_model='estimate'`, `num_iter=7`) gave, across 60 matched scene/channels: mean odds-ratio discrepancy 1.01x (max 1.02x), mean enrichment Δ=0.008 — an order of magnitude smaller than the ~1.2x run-to-run reproducibility floor established elsewhere in this project. Bit depth is not the explanation for the `2026_06_05` gap, and more generally has no meaningful effect on these metrics under this deconvolution setting. Full method/results: `analysis_notebooks/colocalization_metrics_explanation.md`, "Follow-up: does bit depth matter, once the pipeline is fixed?"; reproducible cells in `26_06_15_all_b.ipynb`/`26_06_25_all_b.ipynb`.
